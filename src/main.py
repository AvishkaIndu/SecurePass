...
class LoginWindow(QDialog):
    ...
    def setup_ui(self):
        ...
        # Dark theme
        self.setStyleSheet("""
            QDialog { background-color: #1e1e1e; color: #e0e0e0; font-family: 'Segoe UI', Arial; }
            QLabel { color: #e0e0e0; font-size: 11pt; }
            QLineEdit { background-color: #2d2d2d; border: 2px solid #3d3d3d; border-radius: 6px; padding: 10px; color: #e0e0e0; font-size: 11pt; }
            QLineEdit:focus { border: 2px solid #0d7377; }
            QPushButton { background-color: #0d7377; color: white; border: none; border-radius: 6px; padding: 12px; font-weight: bold; font-size: 11pt; }
            QPushButton:hover { background-color: #14a085; }
            QPushButton:pressed { background-color: #0a5f63; }
            QProgressBar { border: 1px solid #3d3d3d; border-radius: 4px; text-align: center; background-color: #2d2d2d; height: 20px; }
        ...
        # Subtitle
        subtitle_text = "Create Master Password" if self.setup_mode else "Master Password"
        ...
        # Info
        info_text = "First time setup - creating encrypted vault" if self.setup_mode else "Enter your master password to unlock"
    ...
    def handle_login(self):
        ...
        if self.setup_mode:
            ...
            if score < 50:
                reply = QMessageBox.question(self, "Weak Password", "Your password is weak. Continue anyway?", QMessageBox.Yes | QMessageBox.No)
    ...
    def import_backup(self):
        """Import database backup via file dialog"""
        file_path, _ = QFileDialog.getOpenFileName(self, "Select Backup File", "", "Database Files (*.db);;All Files (*)")
        if file_path:
            try:
                shutil.copy(file_path, "securepass.db")
                QMessageBox.information(self, "Backup Restored", "Backup imported successfully!")
            except Exception as e:
                QMessageBox.warning(self, "Error", f"Failed to import backup:\n{str(e)}")
